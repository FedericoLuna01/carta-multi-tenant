...
const CartModal = ({ userSettings }: { userSettings: UserSettings | null }) => {
  ...
  const title = orderId
    ? `Estado de tu orden - #${orderId}`
    : "Resumen de tu orden";
  const description = orderId
    ? "Mira el progreso de tu orden. ¡Ya casi está listo!"
    : "Mira que bien queda tu orden, ¿quieres cambiar algo?";
  ...
  return (
    <Dialog>
      ...
      <DialogContent className="max-w-screen sm:max-w-[500px] md:max-w-[700px] xl:max-w-[800px] max-h-[95vh] overflow-y-auto p-0">
        <DialogHeader className="sticky z-50 px-8 top-0 bg-white py-4 translate-y-[-2px] border-b border-b-slate-200">
